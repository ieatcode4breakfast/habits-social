                ...
                <template v-if="skipsPeriod === 'weekly' || skipsPeriod === 'monthly'">
                  <div class="flex items-start gap-3">
                    <div class="flex items-center gap-3">
                      ...
                      <span class="text-zinc-500 text-sm">{{ skipsCount === 1 ? 'skip' : 'skips' }}</span>
                    </div>
                  </div>
                </template>
              </div>
                ...
                <div class="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  <label v-for="friend in friends" :key="friend.id" class="flex items-center justify-between p-3 bg-black border border-zinc-925 rounded-xl cursor-pointer hover:border-zinc-800 transition-colors">
                    ...
                    <div 
                      ...
                      :class="[sharedWith.includes(friend.id) ? 'bg-zinc-700 shadow-lg shadow-zinc-700/20' : 'bg-zinc-925']"
                    >
                      <Check v-if="sharedWith.includes(friend.id)" class="w-3.5 h-3.5 text-zinc-100" />
                    </div>
