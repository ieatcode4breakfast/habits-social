<template>
  <div class="min-h-[100dvh] antialiased font-sans select-none"><!-- ponytail: runtime guard for a build-time module absence; acceptable because Nuxt module resolution is build-time anyway. --><VitePwaManifest v-if="pwaEnabled" /><ClientOnly><NuxtRouteAnnouncer /></ClientOnly><NuxtLayout><NuxtPage :keepalive="{ include: ['social', 'inbox'] }" /></NuxtLayout><HelpCenterModal /><ClientOnly><Teleport to="body"><Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-4 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95"><div v-if="isVisible" class="fixed bottom-24 md:bottom-12 inset-x-0 mx-auto z-[200] flex items-center justify-center gap-2 px-4 py-2.5 w-max max-w-[calc(100vw-1rem)] bg-surface-solid/90 border border-fg/10 rounded-2xl md:rounded-full backdrop-blur-xl ring-1 ring-fg/5 pointer-events-none"><div class="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-lg" :class="{'bg-emerald-500/20 text-emerald-500': type === 'completed', 'bg-rose-500/20 text-rose-500': type === 'failed', 'bg-zinc-500/20 text-fg-muted': type === 'skipped', 'bg-surface-hover/50 text-fg-subtle': type === 'cleared'}"><CheckIcon v-if="type === 'completed'" class="w-4 h-4" /><XIcon v-else-if="type === 'failed'" class="w-4 h-4" /><MinusIcon v-else-if="type === 'skipped'" class="w-4 h-4" /><Trash2Icon v-else-if="type === 'cleared'" class="w-4 h-4" /></div><span class="text-sm font-bold tracking-tight text-fg text-center">{{ message }}</span></div></Transition></Teleport></ClientOnly></div>
</template>
...
const { pwaEnabled } = config.public;
