...
### 1. `passwordHash` Leaked to Client via `/api/users/me` GET and PUT
- **Location:** `server/api/users/me.get.ts:13-21`, `server/api/users/me.put.ts:69-81`
- **Issue:** Both endpoints use `db.select().from(users)` with zero column projection. The PUT's `.returning()` also returns all columns. Every response sends `passwordHash`, `emailVerifiedAt`, and all internal columns directly in the JSON body. (Note: `auth/me.get.ts` is safe — it already uses explicit column selection.)
- **Fix `me.get.ts`:** Replace the unscoped `db.select()` with explicit column projection:
  ```ts
  // me.get.ts — replace lines 13-15
  ...
  ```
- **Fix `me.put.ts`:** Add `.returning()` with the same projection:
  ```ts
  // me.put.ts — replace lines 69-79
  ...
  ```
- **Fix `me.put.ts` (fetch):** Also fix the fetch on line 27 that reads the current user. It currently uses `db.select().from(users)` which pulls `passwordHash` into `user` — this is needed for the `newPasswordHash` fallback on line 61, so the fetch must include `passwordHash` internally. However, ensure the variable is never returned. The `.returning()` fix above handles this.

### 2. Authorization Bypass in `removeFriendship` — Any User Can Delete Any Friendship
- **Location:** `server/services/social.service.ts:128-141`
- **Issue:** `removeFriendship` fetches the friendship by `id` alone (line 129-131) and never checks that `userId` is either the `initiatorId` or `receiverId`. Any authenticated user who knows or guesses a friendship UUID can delete **any** friendship and trigger `cleanupFriendshipData`, which removes shared habits, bucket memberships, and sharing flags between two completely unrelated users.
- **Fix:** Add an authorization guard after the fetch, before cleanup:
  ```ts
  // social.service.ts — insert after line 135 (const friendship = friendshipsRes[0];)
