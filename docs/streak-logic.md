...
**CRITICAL RULE:** Streaks are ALWAYS calculated on a daily, day-by-day basis, regardless of the habit’s skip budget. The `currentStreak` strictly counts consecutive days of logging activity. The frequency goals (e.g., "4 times a week") use a separate visual counter, but the streak pill badge is universally a daily measure.

> [!IMPORTANT]
**Design Philosophy: The Unbroken Chain**
This daily-only logic is **intentional and immutable**. While habits have skip budgets (weekly/monthly), the **Streak** is a separate measure of daily accountability.
> 
