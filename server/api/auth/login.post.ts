...
export default defineEventHandler(async (event) => {
  ...
  if (!user) throw createError({ statusCode: 400, statusMessage: 'Incorrect username, email or password' });

  const isMatch = await compare(password, user.passwordHash);
  if (!isMatch) throw createError({ statusCode: 400, statusMessage: 'Incorrect username, email or password' });
