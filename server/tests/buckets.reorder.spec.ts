...
describe('POST /api/buckets/reorder', () => {
  let handler: any;
  let testUser: any;
  let bucket1: any;
  let bucket2: any;
  ...
  beforeAll(async () => {
    handler = (await import('../api/buckets/reorder')).default;
  ...
  it('should reorder buckets successfully', async () => {
    ...
    // Verify ordering
    const getHandler = (await import('../api/buckets/index')).default;
  ...
  it('should fail if too many bucket IDs are submitted (max 200)', async () => {
    const manyIds = Array.from({ length: 201 }, () => '00000000-0000-0000-0000-000000000000');
    const event = createMockEvent(testUser.id, {
      ids: manyIds
    }, {}, {}, {}, 'POST');
