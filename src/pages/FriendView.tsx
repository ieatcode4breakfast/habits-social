...
export const FriendView = () => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [h, l] = await Promise.all([
          getFriendHabits(friendId, user.uid),
          getFriendHabitLogs(friendId, user.uid)
        ]);
        setHabits(h);
        setLogs(l);
      } catch (e) {
  ...
  if (loading) return <div className="text-gray-500">Loading friend's habits...</div>;

  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center gap-4">
        <Link to="/social" className="inline-flex items-center justify-center w-10 h-10 rounded-[12px] bg-white border border-gray-200 hover:bg-gray-50 transition-colors">
        ...
        {profile && (
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gray-200 rounded-full flex items-center justify-center overflow-hidden">
              {profile.photoURL ? <img src={profile.photoURL} alt="photo" referrerPolicy="no-referrer" /> : <User className="w-6 h-6 text-gray-500" />}
            </div>
            <div>
              <h1 className="text-[28px] font-bold tracking-tight text-gray-900 mb-1">{profile.displayName}'s Habits</h1>
              <p className="text-gray-500 text-[16px]">Habits shared with you</p>
            </div>
          </div>
        )}
      </div>

      <div className="bg-white border border-gray-200 rounded-[16px] overflow-x-auto tracking-tight text-[14px]">
        <table className="w-full text-left border-collapse min-w-max">
          <thead>
            <tr>
              <th className="p-4 border-b border-gray-200 font-medium text-gray-500 w-full min-w-[150px]">
                <div className="text-[14px] font-[600] uppercase tracking-[0.05em] text-gray-500">Shared Habits</div>
              </th>
              {days.map((day, i) => (
                <th key={i} className="p-4 border-b border-gray-200 text-center font-medium text-gray-500 min-w-[48px]">
                  ...
                  <div className="mt-1 text-gray-900 text-[16px]">{format(day, "d")}</div>
                </th>
              ))}
            </tr>
          </thead>
            ...
            {habits.length === 0 ? (
              ...
              habits.map((habit) => (
                <tr key={habit.id} className="group hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
                  <td className="p-4 font-[600] text-gray-900 flex items-center gap-3 text-[16px]">
